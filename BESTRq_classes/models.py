...
class BiGRU(nn.Module):
    ...
    def __init__(self, input_dim=1, hidden_dim=50, nstack = 3, dropout=0.3):
        ...
        self.nstack = nstack
        self.dropout = dropout
        self.lstm = nn.GRU(input_size = input_dim, hidden_size = hidden_dim, num_layers = nstack, bias=True, batch_first=False, dropout=0.4, bidirectional=True)
        self.drop = nn.Dropout(p = dropout)
    ...
    def forward(self, inputs, h0=None):
        if h0 is None:
            h0 = self.init_hidden(inputs.shape[1])  # Use size(1) to get the batch size

        h0 = h0.to(inputs.device)
        gru_output, _ = self.lstm(inputs, h0)

        out = self.drop(gru_output)
